...
#import "UGHeader.h"
@interface SearchViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UGCustomnNavViewDelegate>{
    UITextField *sfield;
    UIView *leftview;
    UILabel *label;
    UIView *rightview;
...
- (void)viewDidLoad {
   ...
   //导航栏的搜索框
    ...
    _historyarry=[df objectForKey:@"history"];
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    NSUserDefaults *df=[NSUserDefaults standardUserDefaults];
    if ([df objectForKey:@"history"]) {
        arr =[df objectForKey:@"history"];
    ...
    [df setObject:muarr forKey:@"history"];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_dataarray.count) {
       ...
       CGFloat s=model.promotionPrice.floatValue;
        CGFloat vips=model.vipPrice.floatValue;
        if(model.vipPrice!=nil){
        if (s>vips) {
           cell.pricelable.text=[NSString stringWithFormat:@"￥%.1f",vips];
        }else{
       ...
       CGFloat s1=model.goodsPrice.floatValue;
        if (model.saleCount==nil) {
            ...
            NSNumber *sacount=model.saleCount;
            cell.salesLable.text=[NSString stringWithFormat:@"%@人付款",sacount];
    ...
    if ([de objectForKey:@"userId"]) {
        if (_hang==1) {
            ...
            if (YN) {
               ...
               // _dic=@{@"goodsName":_secherstr,@"min":min,@"max":@10,@"sessionid":[de objectForKey:@"sessionid"],@"userId":[de objectForKey:@"userId"],@"model":@1,@"ios_version":VERSION
                    //   };
            }else{
                [mudic setObject:_secherstr forKey:@"barCode"];
                [mudic setObject:min forKey:@"min"];
               ...
               // _dic=@{@"barCode":_secherstr,@"min":min,@"max":@10,@"sessionid":[de objectForKey:@"sessionid"],@"userId":[de objectForKey:@"userId"],@"model":@1,@"ios_version":VERSION
                      // };
            }
        }
